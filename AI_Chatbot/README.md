# AI Chatbot Demo (React + Node.js + Mistral API)
...
- Replies come from Mistral API through a Node/Express backend
...
Go to https://console.mistral.ai/ and create a free API key.
...
- The Mistral API key lives only on the server (`server/.env`) — it is never exposed to the browser. This is intentional and worth pointing out as a security best practice.
- Conversation history is kept in React state and sent with every request, so Mistral has context of the conversation (see `buildHistory` in `ChatWindow.jsx` and `getChatResponse` in `mistral.service.js`).
- Model used: `mistral-large-latest`. You can change this in `server/src/services/mistral.service.js`.

## Common issues

- **"MISTRAL_API_KEY is not set" warning**: you forgot to create `server/.env` from `.env.example`.
